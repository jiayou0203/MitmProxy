...
#            ctx.log("原始数据值是:%s",flow.request.content.decode('utf-8'))
            request_data['method'] = flow.request.method
            request_data['url'] = flow.request.url
#            request_data['data']=flow.request.data
            ctx.log("method 是:%s"%request_data['method'])
            ctx.log("url是:%s"% request_data['url'])

            #遍历请求头信息放入headers数组
            for key, value in flow.request.headers.items():
                headers[key] = value
                ctx.log("headers key is:%s"%key)
                ctx.log("headers value is:%s"%value)
            request_data['headers'] = headers
 #           ctx.log("请求头是:%s"%headers)
        ...
        if flow.request.host=='www.hebeiza.cn':
            ...
            if is_body:
                ...
                request_data.update(body)
            ctx.log("request_data是:%s" % request_data)
            #            request_data['data']=Joker.loads(flow.response.content.decode('utf-8'))
            ...
            if flow.request.host == 'rdi.org.cn':
                ...
                ctx.log("method 是:%s" % request_data['method'])
                ctx.log("url是:%s" % request_data['url'])
                #            ctx.log("请求参数内容是：%s"%request_data['data'])
                # 遍历请求头信息放入headers数组
                for key, value in flow.request.headers.items():
                    headers[key] = value
                request_data['headers'] = headers
